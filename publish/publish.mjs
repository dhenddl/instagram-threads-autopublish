...
const GATES = [
  ...
  // 2026-09-09 추가 (사용자 지시) — 발행일이 오면 뜻이 바뀌는 시점 표현.
  // ⛔ 이 파일만 blog/ 에 있다. 블로그 원고와 **규칙을 공유**하기 때문이다 —
  //   여기 복사본을 두면 시점 규칙이 두 곳이 되고 한 곳만 고쳐진다.
  // 📌 왜 넣었나: 2026-09-09 토스 쿠폰 종료 문자를 보고 예약분을 훑다가
  //   9/16 회차의 「이번엔」·9/17 회차의 「다음 주에」를 찾았다.
  //   ⛔ **사용자 제보로 찾았지 코드가 찾은 게 아니다.** 그래서 코드로 옮겼다.
  // ★ 차단은 「어제·내일」 계열 + 집필→발행 간격 2일 이상뿐이다(고정 포인터라 무조건 어긋난다).
  //   주 단위는 「다음 주에 다시 뵐게요」처럼 발행 시점에도 참인 문장이 많아 **경고**로 둔다.
  //   🔬 낱말만 세면 145건 중 77건이 걸리고 대부분 정상 문체다 — 축은 낱말이 아니라 간격이다.
  ['../blog/check-blog-time.mjs', '발행일과 어긋나는 시점'],
  // 2026-09-10 추가 (사용자 지시) — 자료 배포 회차인데 공개 리포가 낡은 것.
  // 📌 왜 넣었나: 9/10 에 공개 리포 넷을 전수 대조하니 **셋이 낡아 있었다.**
  //   그중 2호는 배포일이 9/07 인데 리포 마지막 커밋이 8/27 이었다 — 변경은 9/02 에
  //   이미 있었으니 **그날 자료를 받은 사람은 그 변경이 없는 코드를 받았다.**
  //   ⛔ 링크는 살아 있었고 리포도 돌았다. `check-blog-url` 은 「주소가 사나」를 묻지
  //      「내용이 최신인가」를 안 묻는다 — 아무 신호가 없던 이유다.
  // ★ **매니페스트가 `"material": "3호"` 로 스스로 선언한 회차만** 본다.
  //   ⛔⛔ 날짜로 걸지 않았다 — 배포일에는 07:00 AM 스레드도 09:00 토스도 같이 있어서
  //      리포 드리프트로 **그날 아침 스레드가 안 나가는** 사고가 된다.
  // ⚠️ 이 게이트와 `check-blog-url` 둘만 네트워크를 탄다. 클론·내보내기 실패는
  //   **경고하고 통과**시킨다 — 못 물어본 것과 없는 것은 다르다.
  ['check-material-drift.mjs', '자료 배포본이 낡았나'],
];
if (args.skipGates) {
